...
export async function POST(request: Request) {
  try {
    ...
    let materiasQuery = supabase.from("materias_mayo_agosto").select("*")
    ...
    const { error: deleteError } = await supabase.from("asignaciones_mayo_agosto").delete().in("grupo_id", grupoIds)
